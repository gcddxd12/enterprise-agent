...
from resilience import get_logger, retry_with_backoff, classify_llm_error
logger = get_logger("cmcc_agent.agent")


@retry_with_backoff(max_retries=3, base_delay=1.0)
def _safe_llm_invoke(llm_instance, input_messages):
    """Invoke LLM with retry on transient failures (timeout, rate limit)."""
    try:
        return llm_instance.invoke(input_messages)
...
try:
    ...
    logger.warning(f"高级RAG系统导入失败，将使用模拟模式: {e}")
...
try:
    ...
    logger.warning(f"监控系统导入失败，监控功能将不可用: {e}")
    MONITORING_AVAILABLE = False
    monitoring_system = None
...
def init_advanced_rag():
    ...
    try:
        ...
        if os.path.exists(chroma_db_path):
            ...
            try:
                ...
                if collection:
                    ...
                    if results and 'documents' in results:
                        ...
                        logger.info(f"从向量数据库加载了 {len(documents_for_bm25)} 个文档用于BM25检索")
            except Exception as e:
                logger.warning(f"无法从向量数据库获取文档: {e}")
                # 如果无法获取文档，使用一些默认文档
                documents_for_bm25 = [
                    Document(
            ...
            logger.warning("向量数据库不存在，使用模拟模式")
            return None
    except Exception as e:
...
def knowledge_search(query: str) -> str:
    ...
    if MONITORING_AVAILABLE and monitoring_system:
        try:
            ...
            logger.warning(f"RAG检索监控跟踪失败: {e}")

    # 如果有高级RAG检索器，使用它
    if advanced_rag_retriever:
        try:
            logger.info(f"使用高级RAG检索查询: '{query}'")
            ...
            if results and len(results) > 0:
                ...
                if MONITORING_AVAILABLE and monitoring_system:
                    try:
                        ...
                        logger.warning(f"RAG检索成功监控跟踪失败: {e}")

                return adapted_response
            else:
                logger.warning("高级RAG检索到0个结果，回退到模拟模式")
                # 监控跟踪：RAG检索无结果
                if MONITORING_AVAILABLE and monitoring_system:
                    try:
                        monitoring_system.track_rag_retrieval(query, duration, 0, success=False, error="No results found")
                    except Exception as e:
                        logger.warning(f"RAG检索无结果监控跟踪失败: {e}")
        except Exception as e:
            duration = time.time() - start_time
            logger.error(f"高级RAG检索失败: {e}，回退到模拟模式")
            # 监控跟踪：RAG检索失败
            if MONITORING_AVAILABLE and monitoring_system:
                try:
                    monitoring_system.track_rag_retrieval(query, duration, 0, success=False, error=str(e)[:100])
                except Exception as e2:
                    logger.warning(f"RAG检索失败监控跟踪失败: {e2}")

    # 回退到内存知识库仓储
    logger.info(f"使用内存知识库检索查询: '{query}'")
    try:
...
def agent_node(state: AgentState) -> AgentState:
    ...
    logger.info(f"初始消息数: {len(messages)}")
    ...
    for step_idx in range(MAX_AGENT_STEPS):
        logger.info(f"ReAct 第 {step_idx + 1} 步...")
        step_start = time.time()

        response = _safe_llm_invoke(llm_with_tools, messages)
        ...
        if not response.tool_calls:
            ...
            logger.info(f"LLM 完成（无工具调用），答案长度: {len(final_answer)}, 耗时: {step_duration:.2f}s")

            duration = time.time() - start_time
            logger.info(f"ReAct 完成，共 {step_idx + 1} 步，总耗时: {duration:.2f}s")
            logger.info(f"调用工具: {plan}")
            return {
                **state,
        ...
        for tool_call in response.tool_calls:
            ...
            logger.info(f"调用工具: {tool_name}, 参数: {str(tool_args)[:80]}")
            ...
            try:
                ...
                result = f"工具执行失败: {e}"
                logger.info(f"工具 {tool_name} 执行异常: {e}")

            tool_results[f"{tool_name}: {query_str[:50]}"] = result[:500]
            messages.append(ToolMessage(content=result, tool_call_id=tool_id))

        logger.info(f"第 {step_idx + 1} 步完成，调用: {tool_names}，耗时: {step_duration:.2f}s")

    # 达到最大步数，强制输出
    logger.info(f"达到最大步数 {MAX_AGENT_STEPS}，强制输出最终回复")
    llm_direct = get_llm()
...
请直接输出客服回复："""
    try:
        final_answer = _safe_llm_invoke(llm_direct, [HumanMessage(content=force_prompt)]).content
    except Exception:
        final_answer = "抱歉，处理您的问题需要更多时间，建议您拨打10086热线或前往就近营业厅咨询。"

    duration = time.time() - start_time
    logger.info(f"ReAct 完成（达最大步数），总耗时: {duration:.2f}s")
    return {
        **state,
        "final_answer": final_answer,
        "raw_context": raw_context,
...
def preprocess_node(state: AgentState) -> AgentState:
    ...
    if MONITORING_AVAILABLE and monitoring_system:
        try:
            ...
            logger.warning(f"监控跟踪失败: {e}")

    logger.info(f"对话摘要: {conversation_summary}")
    ...
    try:
        ...
        if matching_skills:
            ...
            logger.info(f"激活Skill: {active_skills}")
    except Exception as e:
        logger.info(f"Skill匹配失败: {e}")
...
请直接输出面向用户的客服回复（自然语言，语气友好专业，不超过300字）："""
            llm = get_llm()
            synthesized = _safe_llm_invoke(llm, [HumanMessage(content=synthesis_prompt)]).content
            if synthesized and len(synthesized.strip()) > 10:
                final_answer = synthesized.strip()
                logger.info(f"LLM已整理答案（原{len(raw_answer)}字 → {len(final_answer)}字）")
            else:
                final_answer = raw_answer
        except Exception as e:
            logger.info(f"LLM整理失败，使用原始答案: {e}")
            final_answer = raw_answer
    else:
        final_answer = raw_answer
...
def visualize_workflow(output_path: str = "workflow_with_memory.png"):
    ...
    try:
        ...
        with open(output_path, "wb") as f:
            f.write(png_data)
        logger.info(f"工作流图已保存至: {output_path}")
        ...
        logger.info(f"Mermaid 代码已保存至: {mermaid_path}")
